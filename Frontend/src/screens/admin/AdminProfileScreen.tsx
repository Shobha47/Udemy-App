...
import { instructorApi } from '../../api/instructor.api'; // Reuses shared /auth/me profile fetcher
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <View style={styles.headerContainer}>
        <Text style={styles.headerTitleText}>Admin Control Desk</Text>
        <TouchableOpacity 
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.heroSection}>
          <Image
            source={{ 
              uri: user.avatar || `https://ui-avatars.com/api/?name=${user.name}&background=7C3AED&color=fff&size=200` 
            }}
            style={styles.avatarImage}
          />
          <Text style={styles.adminNameText}>{user.name}</Text>
          <Text style={styles.adminEmailText}>{user.email}</Text>
          
          <View style={styles.roleBadgeFrame}>
            <View style={styles.pulseDotIndicator} />
            <Text style={styles.roleBadgeText}>SYSTEM SUPERADMIN</Text>
          </View>
        ...
        {(user.website || socialLinks.twitter || socialLinks.linkedin || socialLinks.youtube) ? (
          <View style={styles.socialSectionBordered}>
            <Text style={styles.sectionTitle}>Corporate Footprint Channels</Text>
            <View style={styles.socialRowGroup}>
              {user.website && (
                <TouchableOpacity style={styles.socialPillBtn} onPress={() => handleOpenURL(user.website)}>
                  ...
                  <Text style={styles.socialPillText}>Corporate</Text>
                </TouchableOpacity>
              )}
        ...
        <View style={styles.flatSectionBordered}>
          <Text style={styles.sectionTitle}>Administrative Scope / Bio</Text>
          <Text style={styles.biographyParagraphText}>
            {user.bio || "No custom admin scope narrative mapped to this profile yet. Use edit screen to customize."}
          </Text>
        </View>
        ...
        <View style={{ paddingHorizontal: 20, marginTop: 32 }}>
          <TouchableOpacity style={styles.systemLogoutBtn} onPress={logout}>
            ...
            <Text style={styles.systemLogoutBtnText}>Shutdown Terminal Session</Text>
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  adminNameText: { fontSize: 22, fontWeight: '800', color: '#0F172A', marginTop: 14, letterSpacing: -0.5 },
  adminEmailText: { fontSize: 13, color: '#64748B', marginTop: 2 },
  roleBadgeFrame: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#7C3AED', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 6, marginTop: 12 },
