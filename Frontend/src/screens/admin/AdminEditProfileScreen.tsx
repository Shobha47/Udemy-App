// src/screens/admin/AdminEditProfileScreen.tsx
...
export default function AdminEditProfileScreen({ navigation }: any) {
  const queryClient = useQueryClient();

  // --- ADMIN DATA ATTRIBUTES STATES ---
  ...
  const { data: user, isLoading } = useQuery({
    queryKey: ['admin-profile-specs'],
    queryFn: instructorApi.getProfile,
  });

  // Rehydrate values smoothly on initialization cycles
  useEffect(() => {
    if (user) {
      setName(user.name ?? '');
  ...
  // --- CORE CONSOLE UPDATE PIPELINE MUTATION ---
  const updateProfileMutation = useMutation({
    mutationFn: instructorApi.updateProfile,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-profile-specs'] });
      Alert.alert('Success', 'System administrative parameters aligned cleanly.');
      navigation.goBack();
    },
    onError: (err: any) => {
      Alert.alert('Sync Failure', err?.message || 'Failed to sync modifications.');
    }
  });
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitleText}>Edit System Profile</Text>
        <View style={{ width: 24 }} />
      </View>

      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.formContextContainer}>
          <Text style={styles.formGroupSectionHeading}>Identity Specifications</Text>

          {/* Name Field Input */}
          <View style={styles.inputFieldGroup}>
            <Text style={styles.fieldLabelText}>Corporate Operator Name</Text>
            <TextInput style={styles.textInputNode} value={name} onChangeText={setName} placeholder="Your name" placeholderTextColor="#94A3B8" />
          </View>

          {/* Avatar Field Input */}
          <View style={styles.inputFieldGroup}>
            <Text style={styles.fieldLabelText}>Avatar Asset Link (URL)</Text>
          ...
          <View style={styles.inputFieldGroup}>
            <Text style={styles.fieldLabelText}>Professional Headline / Operations Scope</Text>
            <TextInput style={styles.textInputNode} value={headline} onChangeText={setHeadline} placeholder="e.g., Chief Education Officer / System Operations Lead" placeholderTextColor="#94A3B8" />
          </View>

          {/* Biography Field Input */}
          <View style={styles.inputFieldGroup}>
            <Text style={styles.fieldLabelText}>Administrative Scope Statement</Text>
            <TextInput 
              ...
              value={bio} 
              onChangeText={setBio} 
              placeholder="Map specific operations credentials and access scope items..."
          ...
          {/* Corporate Website */}
          <View style={styles.inputFieldGroup}>
            <Text style={styles.fieldLabelText}>Corporate Portal Link (URL)</Text>
            <TextInput style={styles.textInputNode} value={website} onChangeText={setWebsite} autoCapitalize="none" keyboardType="url" placeholder="https://domain.com" placeholderTextColor="#94A3B8" />
          </View>

          {/* LinkedIn Channel link */}
          <View style={styles.inputFieldGroup}>
            <Text style={styles.fieldLabelText}>LinkedIn Network Destination</Text>
          ...
          {/* Twitter Link */}
          <View style={styles.inputFieldGroup}>
            <Text style={styles.fieldLabelText}>Twitter Handle Platform Link</Text>
      ...
      {/* ATTACHED STICKY LOWER BUTTON ACTION SHEET CONTAINER */}
        ...
        >
          {updateProfileMutation.isPending ? (
            ...
            <>
              ...
              <Text style={styles.actionSheetSubmitBtnText}>Synchronize Terminal Profiles</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  actionSheetSubmitBtn: { backgroundColor: '#7C3AED', height: 48, borderRadius: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 },
